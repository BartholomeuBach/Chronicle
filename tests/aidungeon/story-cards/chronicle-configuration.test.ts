...
describe("Chronicle Configuration Story Card", () => {
  ...
  it("defaults an existing card to enabled automatic mode and allows pause", () => {
    expect(readChronicleConfiguration([card("")])).toEqual({ enabled: true, mode: "automatic", repairChronicleCard: false, aiTemporalSignal: true });
    expect(readChronicleConfiguration([card("Chronicle Enabled: false")])).toEqual({ enabled: false, mode: "automatic", repairChronicleCard: false, aiTemporalSignal: true });
  });
  it("disables the AI-signaled evidence tier only on explicit request", () => {
    expect(readChronicleConfiguration([card("AI Temporal Signal: false")])).toMatchObject({ aiTemporalSignal: false });
    expect(readChronicleConfiguration([card("")])).toMatchObject({ aiTemporalSignal: true });
  });
