...
const modifier = (value: string) => ({
  text: globalThis.ChronicleAIDungeon?.onContext(value, { state, actionCount: info.actionCount, maxChars: info.maxChars, memoryLength: info.memoryLength, storyCards: { storyCards, addStoryCard, updateStoryCard } }) ?? nonEmptyText(value)
});
